...
function data_voisines(scope){
	...
	for (var h=1;h<=scope.height;h++){
		for (var w=1;w<=scope.width;w++){
			if(scope.box[h][w].bomb){			//si cette case est une bombe,on le passer
...
function event_clickBox(e){
	...
	if(!this.isStart){event_start(this)}		//si le jeu pas commence, call la methode event_start
	var h=parseInt(e.target.getAttribute("h"));
	var w=parseInt(e.target.getAttribute("w"));	//les deux ligne pour savoir le utilisateur a click quelle case
	...
	if(e.button==2){							//click droit
		...
		if(this.box[h][w].bomb&&!this.box[h][w].marque){//si c'est une case bombe et on n'a pas marqué
			event_Boum(h,w,this);				//call la methode event_bombe
		}else{
			control_openBox(h,w,this);			//sinon call la methode control_openbox
		}
	}
...
function event_end(scope){
	var end_time= new Date();		//obtenir un temps
	scope.time	= parseInt(parseInt(end_time - scope.start_time)/1000);	//calculer on a utiliser combien de temps
	scope.isOver= true;				//marque ce jeu est terminé
}

//il marche quand on click une bombe
function event_Boum(h,w,scope){
	event_end(scope);		//call la methode event_end
	//parcourir la table de case
	for (var fh=1;fh<=scope.height;fh++){
		for (var fw=1;fw<=scope.width;fw++){
			if(scope.box[fh][fw].bomb&&!scope.box[fh][fw].marque){
				scope.td[fh][fw].setAttribute("class","bombe");				//toutes les cases de bombes affichent
			}else if(scope.box[fh][fw].marque&&!scope.box[fh][fw].bomb){
				scope.td[fh][fw].setAttribute("class","erreur");			//afficher les fautes cases
			}
		}
	}
	scope.td[h][w].setAttribute("class","boum");		//la case qu'on a préssé afficher une images de exploseur
	scope.onFailed(scope.time);							// call la methode onfailed de objet demineur q'on a prévue
}

//il marche quand on reussir le jeu
function event_Win(scope){
	event_end(scope);		//call la methode event_end
	//parcourir la table de case
	for (var h=1;h<=scope.height;h++){
		for (var w=1;w<=scope.width;w++){
			if(scope.box[h][w].bomb){
				scope.td[h][w].setAttribute("class","marque");			//marquer toutes les cases de bombe
			}else{
				scope.td[h][w].setAttribute("class","vide"+scope.box[h][w].voisine);	//ouvrir tous les cases de normal
			}
		}
	}
	scope.onWin(scope.time);						// call la methode onxin de objet demineur q'on a prévue
}
...
function control_openBox(h,w,scope){
	control_creatOpenList(h,w,scope);	//call la methode control_creatOpenList pour prépare la liste de ouvrir
	var h,w;
	var length=scope.waitOpen.length;
	...
	scope.numOpen	+= length;		//reindique la chiffre de case ouvrir
	if(scope.numOpen==scope.tout){	
		event_Win(scope);			//si la nombre de case ouvrir autant de la nombre de case normal, call la methode event_win
	}
}

//prépqre la liste pour ouvrir selon l'attribute de nombre de voisin
function control_creatOpenList(h,w,scope){
	if(scope.box[h][w].cache&&!scope.box[h][w].marque){		//si la case n'avait pas ouvrir et n'avait pas marque comme une bombe
